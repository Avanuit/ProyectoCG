...
class Particle {
    ...
    draw(cameraOffset) {
        ...
        // Partículas en forma de estrellas
        ctx.beginPath();
...
function createPlatform(yPos) {
    ...
    if (score > 25 && rand < 0.2) {  // probabilidad espinas
        hazards.push({
            x: x + width/2 - 15, 
            y: yPos - 45, 
            w: 30, h: 45, 
...
function triggerWin() {
    ...
    subMsgEl.textContent = `¡Sobreviviste!`;
    overlayEl.classList.add('visible');
}
